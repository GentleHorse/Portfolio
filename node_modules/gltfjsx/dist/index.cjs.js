  ...
  function print(obj, silent = false) {
    ...
    if (instanced) {
      ...
      if (obj.isInstancedMesh) {
        ...
        if (type === 'bone') result = `<primitive object={${node}} `;else result = `<${type} `;
      }
    }
    ...
    if (children.length) {
      if (type === 'bone') result += children + `</primitive>`;else result += children + `</${type}>`;
    }
    return result;
  }
...
${parseExtras(gltf.parser.json.asset && gltf.parser.json.asset.extras)}*/`;
  ...
  const result = `${options.types ? `\nimport * as THREE from 'three'` : ''}
        ...
        ${hasPrimitives || options.types ? `import { ${options.types ? 'GLTF,' : ''} ${hasPrimitives ? 'SkeletonUtils' : ''} } from "three-stdlib"` : ''}
        ...
        export ${options.exportdefault ? 'default' : ''} function Model(props${options.types ? ": JSX.IntrinsicElements['group']" : ''}) {
          ${hasInstances ? 'const instances = React.useContext(context);' : ''} ${hasAnimations ? `const group = ${options.types ? 'React.useRef<THREE.Group>()' : 'React.useRef()'};` : ''} ${!options.instanceall ? `const { ${!hasPrimitives ? `nodes, materials${hasAnimations ? ', animations' : ''}` : 'scene'}} = useGLTF('${url}'${options.draco ? `, ${JSON.stringify(options.draco)}` : ''})${!hasPrimitives && options.types ? ' as GLTFResult' : ''}${hasPrimitives ? `\nconst clone = React.useMemo(() => SkeletonUtils.clone(scene), [scene])
                const { nodes, materials${hasAnimations ? ', animations' : ''} } = useGraph(clone) ${options.types ? ' as GLTFResult' : ''}` : ''}` : ''} ${printAnimations(animations)}
          return (
...
useGLTF.preload('${url}')`;
  ...
  const output = header + '\n' + result;
  const formatted = prettier__namespace.format(output, {
    semi: false,
